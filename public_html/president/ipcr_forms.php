...
// Initialize variable for college
$current_user_college = null;

// Check if both semester and user ID exist
if ($id_of_semester && $idnumber) {
    // Step 1: Fetch the college of the user
    $userCollege_sql = "SELECT college FROM usersinfo WHERE idnumber = ?";
    $userCollege_stmt = $conn->prepare($userCollege_sql);
    $userCollege_stmt->bind_param("s", $idnumber);
    $userCollege_stmt->execute();
    $userCollege_stmt->bind_result($current_user_college);
    $userCollege_stmt->fetch();
    $userCollege_stmt->close();

    // Check if college was found
    if (!$current_user_college) {
        echo "No college found for the given ID number.";
        exit; // Stop execution if no college is found
    }

    // Step 2: Fetch College Dean (Office Head)
...
// Check if the query for College Dean executed successfully
if ($collegeDean_result === false) {
    die("Error executing query: " . $conn->error);
}

// Fetch the College Dean's information if found
if ($collegeDean_result->num_rows > 0) {
    ...
    echo "No College Dean found for the college: " . htmlspecialchars($current_user_college);
}
...
// Fetch parameters from the GET request
$semester_id = isset($_GET['id_of_semester']) ? htmlspecialchars($_GET['id_of_semester']) : null; // Ensure you have the semester ID
$idnumber = isset($_GET['idnumber']) ? htmlspecialchars($_GET['idnumber']) : null; // Fetch user ID from GET request

// Check if both semester and user ID exist
if ($semester_id && $idnumber) {
    // Fetch performance message
    $message_sql = "SELECT message FROM performance_ipcr_message WHERE idnumber = ? AND semester_id = ?";
    $message_stmt = $conn->prepare($message_sql);
    $message_stmt->bind_param("ss", $idnumber, $semester_id); // Use $idnumber and $id_of_semester
    $message_stmt->execute();
    $message_stmt->bind_result($performance_message);
    $message_stmt->fetch();
    $message_stmt->close();
...
function exportToWord() {
    ...
    <td style="position: relative; padding: 0 0 0 0; height: 75px; border-bottom: none;">
                ...
                <?php 
                echo !empty($deans_first_signature) ? $deans_first_signature : '&nbsp;'; // Use a non-breaking space to maintain width
                ?>
                    </td>
                    <td style="position: relative; padding: 0 0 0 0; height: 75px; border-bottom: none;">
                    ...
                    <td rowspan="2" style="padding: 0; margin: 0; "> 
                        <?php 
                            echo !empty($president_final_sign_created) ? $president_final_sign_created : '&nbsp;'; // Use a non-breaking space to maintain width
                        ?>
                    </td>
                </tr>
                <tr>
            ...
            <td class="for-signature" style="position: relative; padding: 0; height: 140px; width: 190px; overflow: hidden; border-bottom: none;">
                ...
                <td class="for-signature" style="position: relative; padding: 0; height: 140px; width: 190px; overflow: hidden; border-bottom: none; vertical-align: top; white-space: normal;">
                    ...
                    <?php if (isset($signature_data_office_head_final) && $signature_data_office_head_final): ?>
                            <img src="data:image/png;base64,<?php echo base64_encode($signature_data_office_head_final); ?>" alt="Signature" style="position: absolute; top: 50%; left: 50%; width: auto; height: auto; max-width: 100%; max-height: 100%; transform: translate(-50%, -50%); z-index: -1;">
                        <?php else: ?>
                            <p style="text-align: center;"></p>
                        <?php endif; ?>
                    </div>
