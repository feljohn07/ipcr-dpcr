    ...
    function updateTasks($conn, $task_type, $task_names, $descriptions, $documents_required, $due_dates, $college, $end_date, $semester_id) {
        ...
        foreach ($existing_task_ids as $index => $task_id) {
            if (isset($task_names[$index])) {
                ...
                // Update the task_assignments table, keeping the original format
                $task_type_value = ($task_type === 'strategic') ? 'strategic' : (($task_type === 'core') ? 'core' : 'support');
                $update_assignments_stmt = $conn->prepare("UPDATE task_assignments SET task_name = ?, task_description = ?, end_date = ?, due_date = ? WHERE idoftask = ? AND semester_id = ? AND task_type = ?");
                if (!$update_assignments_stmt) {
                    die('Prepare failed: ' . $conn->error);
                }
                $update_assignments_stmt->bind_param("sssissi", $task_name_original, $description_original, $end_date, $due_date, $task_id, $semester_id, $task_type_value);
        ...
        for ($i = $new_task_index; $i < count($task_names); $i++) {
            ...
            $stmt->bind_param("ississs", $semester_id, $task_name_table, $description_table, $documents_req, $college, $end_date, $due_date);
            if (!$stmt->execute()) {
                die('Execute failed: ' . $stmt->error);
            }
            ...
            $task_type_value = ($task_type === 'strategic') ? 'strategic' : (($task_type === 'core') ? 'core' : 'support');
            $insert_assignments_stmt = $conn->prepare("INSERT INTO task_assignments (idoftask, semester_id, task_name, task_description, task_type, end_date, due_date) VALUES (?, ?, ?, ?, ?, ?, ?)");
            if (!$insert_assignments_stmt) {
                die('Prepare failed: ' . $conn->error);
            }
            $insert_assignments_stmt->bind_param("iisssss", $last_inserted_id, $semester_id, $task_name_original, $description_original, $task_type_value, $end_date, $due_date);
